...
class segmentation_exporter():
    def __init__(self):
        self.batched_array = None
        self.queue = Queue()

    def add_to_queue(self, segmentation: np.ndarray, output_file: str):
        self.queue.put({'output_file': output_file, 'segmentation': segmentation})
    ...
    def return_batch(self):
        ...
        sorted_items = sorted(items, key=lambda x: x['output_file'])
        self.batched_array = items[0]['segmentation']

        for i in range(1, len(sorted_items)):
            self.batched_array = np.concatenate((self.batched_array, sorted_items[i]['segmentation']), 0)
        
        return self.batched_array
...
def export_prediction_from_logits(predicted_array_or_file: Union[np.ndarray, torch.Tensor], properties_dict: dict,
                                  ...
                                  save_probabilities: bool = False, queue: Queue = None) -> None:
    ...
    if isinstance(dataset_json_dict_or_file, str):
        dataset_json_dict_or_file = load_json(dataset_json_dict_or_file) # load dataset json file
    ...
    queue.put({'output_file': output_file_truncated, 'segmentation': segmentation_final})
