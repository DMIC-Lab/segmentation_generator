...
class nnUNetPredictor(object):
    ...
    def _manage_input_and_output_lists(self, list_of_lists_or_source_folder: Union[str, List[List[str]]],
        ...
        print('managing input and output lists')
        if isinstance(list_of_lists_or_source_folder, str):
            print('list of lists or source folder is a string')
            list_of_lists_or_source_folder = create_lists_from_splitted_dataset_folder(list_of_lists_or_source_folder,
                                                                                       self.dataset_json['file_ending'])
            print('created list of lists from source folder')
        ...
        if not overwrite and output_filename_truncated is not None:
            ...
            print(f'overwrite was set to {overwrite} (True is default), so I am only working on cases that haven\'t been predicted yet. '
    ...
    def predict_from_files(self,
        ...
        self.lists_of_lists = list_of_lists_or_source_folder
    ...
    def predict_from_list_of_npy_arrays(self,
                                        ...
                                        save_probabilities: bool = False,
                                        num_processes_segmentation_export: int = default_num_processes):
        self.lists_of_lists = [[i] for i in range(len(image_or_list_of_images))]
                    ...
                    r.append(
                        export_pool.starmap_async(
                            ...
                            ((prediction, properties, self.configuration_manager, self.plans_manager,
                              self.dataset_json, ofile, save_probabilities, self.queue),)
                        )
                    )
                else:
                    # convert_predicted_logits_to_segmentation_with_correct_shape(
    ...
    def grab_raw_images(self):
        ...
        sorted_lists = sorted(self.lists_of_lists, key=lambda x: x[0])
        self.images = rw.read_images(sorted_lists)
...
def predict_entry_point(i, d, o='', p='nnUNetPlans', tr='nnUNetTrainer', c='nnUNetTest', f=(0, 1, 2, 3, 4), step_size=0.5, disable_tta=False, verbose=False, save_probabilities=False, continue_prediction=False, chk='checkpoint_final.pth', npp=3, nps=3, prev_stage_predictions=None, num_parts=1, part_id=0, device='cuda', disable_progress_bar=False):
    ...
    if device == 'cpu':
        ...
        # multithreading in torch doesn't help nnU-Net if run on GPU
        # check if threads have already been set
        if not threads_set:
            torch.set_num_threads(1)
            torch.set_num_interop_threads(1)
            threads_set = True
    ...
    predictor.predict_from_files(i, o, save_probabilities=save_probabilities,
...
def predict_using_numpy(i, d, properties=None, o='', p='nnUNetPlans', tr='nnUNetTrainer', c='nnUNetTest', f=(0, 1, 2, 3, 4), step_size=0.5, disable_tta=False, verbose=False, save_probabilities=False, continue_prediction=False, chk='checkpoint_final.pth', npp=3, nps=3, prev_stage_predictions=None, num_parts=1, part_id=0, device='cuda', disable_progress_bar=True):
    ...
    predictor.predict_from_list_of_npy_arrays(i, None, properties, None, num_processes=3)
