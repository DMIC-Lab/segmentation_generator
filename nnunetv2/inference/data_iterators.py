...
def preprocessing_iterator_fromfiles(list_of_lists: List[List[str]],
    ...
    done_event = Event()
    abort_event = Event()
    p = preprocess_fromfiles_save_to_queue(list_of_lists, list_of_segs_from_prev_stage_files,
                                             output_filenames_truncated, plans_manager, dataset_json,
                                             configuration_manager, queue, done_event, abort_event, verbose)
...
def preprocessing_iterator_fromnpy(list_of_images: List[np.ndarray],
    ...
    context = multiprocessing.get_context('spawn')
    manager = Manager()
    truncated_ofnames = [f'{i}' for i in range(len(list_of_images))]
    print("truncated_ofnames", truncated_ofnames)
